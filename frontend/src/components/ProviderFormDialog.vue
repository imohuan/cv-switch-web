    ...
    <template #default>
      <div class="grid grid-cols-2 gap-ax-md">
        ...
        <div>
          ...
          <div class="flex gap-ax-sm">
            <AxModelSelect v-model="formModel" :options="fetchedModels"
              :provider-id="editingProvider?.id"
              :base-url="!editingProvider ? formBaseUrl : undefined"
              :api-key="!editingProvider ? formApiKey : undefined"
              placeholder="选择或输入模型" size="lg" class="flex-1" />
            <AxButton size="lg" icon="refresh" :loading="fetchingModels"
              :disabled="!editingProvider && !formBaseUrl"
        ...
        <div class="col-span-2 bg-surface-container-low border border-outline-variant rounded-lg p-ax-md space-y-ax-sm">
          ...
          <div class="space-y-ax-sm">
            ...
            <AxModelSelect v-model="formCodexCatalogModels" :options="fetchedModels" multiple
              placeholder="选择 Codex 模型..." size="lg" />
          </div>
        </div>
